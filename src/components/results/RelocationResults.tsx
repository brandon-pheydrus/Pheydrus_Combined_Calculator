...
function getNatureBadgeClass(nature: 'benefic' | 'malefic' | 'neutral'): string {
  if (nature === 'benefic') {
    return 'text-xs rounded bg-emerald-100 text-emerald-700 px-2 py-1';
  }
  if (nature === 'malefic') {
    return 'text-xs rounded bg-rose-100 text-rose-700 px-2 py-1';
  }
  return 'text-xs rounded bg-slate-100 text-slate-600 px-2 py-1';
}
  ...
  if (!result) {
    return (
      <ResultSection title="Relocation Analysis" defaultOpen={true}>
        <p className="text-gray-400">No relocation data available</p>
      </ResultSection>
    );
  }
    ...
    <ResultSection title="Relocation Analysis" defaultOpen={true}>
      <div className="space-y-6">
        ...
        {result.angularHits && result.angularHits.length > 0 && (
          <div>
            <h3 className="text-base font-semibold text-[#2d2a3e] mb-3">Angular Hits</h3>
            ...
            <ul className="space-y-2">
              {result.angularHits.map((hit, idx) => (
                <li
                  ...
                  className={
                    hit.isCareer
                      ? 'flex flex-wrap items-center justify-between gap-3 rounded border border-amber-200 bg-amber-50 px-3 py-2'
                      : 'flex flex-wrap items-center justify-between gap-3 rounded border border-gray-200 px-3 py-2'
                  }
                >
                  <div className="text-sm text-[#2d2a3e]">
                    ...
                    <span className="opacity-60"> (house {hit.house})</span>
                    {hit.isCareer && (
                      <span className="ml-2 text-xs rounded bg-amber-200 text-amber-800 px-2 py-0.5">
                        career
                      </span>
                    )}
                  </div>
          ...
          <div>
            ...
            <ul className="space-y-2">
              {result.businessHouseActivations.map((activation, idx) => (
                <li
                  key={idx}
                  className="flex flex-wrap items-center justify-between gap-3 rounded border border-gray-200 px-3 py-2"
                >
                  <div className="text-sm text-[#2d2a3e]">
